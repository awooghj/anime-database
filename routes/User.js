    ...
    if (user) {
      ...
      newUser.save((err) => {
        if (err) {
          res.status(500).json({
            message: {
              msgBody: "The username or password can't be empty",
              msgError: true,
            },
          });
        } else {
  ...
  (req, res) => {
    ...
    subscribe.save((err) => {
      if (err)
        ...
        req.user.subscribes.push(subscribe);
    ...
    User.findById({ _id: req.user._id })
      ...
      .exec((err, document) => {
        if (err)
          ...
          res
            .status(200)
            .json({ subscribes: document.subscribes, authenticated: true });
        }
      });
  }
);
...
userRouter.delete("/subscribe/:id", async (req, res, next) => {
